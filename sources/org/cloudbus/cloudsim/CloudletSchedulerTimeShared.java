...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			//System.out.println("Value of MIPS share is " +mipsShare.get(0));
			
			double estimatedFinishTime = currentTime
					+ (rcl.getRemainingCloudletLength() / (getCapacity(mipsShare) * rcl.getNumberOfPes()));
			if(rcl.getCloudletId() == 21){
				CloudSim.clock();
			//	System.out.println("Estimated Finishing Time is " +estimatedFinishTime);
			//	System.out.println("Turn Around time is " +(estimatedFinishTime - rcl.getCloudlet().getExecStartTime()));
			}
	...
	protected <T extends ResCloudlet> List<T> getCloudletExecList() { //Made public by Yogesh Sharma
